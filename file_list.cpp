...
//FROM ME: it left pads with zeroes to fill out the specified length but not sure exactly how it is doing it.  I will
//not worry about this for now as I feel I understand how it works.
static string make_filename(const string file_name, string::size_type start, string::size_type len, unsigned long value)
{
    ...
    sprintf(format_descriptor, "%%0%02ulu", static_cast<unsigned>(len));
...
//FROM ME: I assume that the stride is always constant in the # sequence
//of the filenames.  
bool  file_list(const string &file_name,QVector<string> * file_list)
{
